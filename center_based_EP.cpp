...
center_based_EP::center_based_EP(void)
{
	maximum_capacity = 20;
...
void center_based_EP::greedy_schedule(int i){
	vector<int>temp;
...
double center_based_EP::calculate_time_difference(eachAirplane prior, eachAirplane next,double safe_time){
...
void center_based_EP::setSortedconf(int n,double time){
	sortedConf.at(n).setSortedentrancetime(time);
}


bool center_based_EP::kickOverflow(){
...
bool center_based_EP::kickUnsafe(int i){
	vector<int>temp;
	vector<vector<int>>temp_generation = getGeneration_pool();
	temp = temp_generation[i];
	for(int i=0;i<temp.size();i++){
...
double center_based_EP::fitness_calculation(){
	double delay = 0.0;
	for(int i=0;i<sortedConf.size();i++){
...
void center_based_EP::displaySortednumber(){
	for(int i=0;i<sortedNumber.size();i++)
		cout<<sortedNumber[i]<<" ";
}

vector<double> center_based_EP::occupation_ratio(){
	vector<vector<int>>temp_generation = getGeneration_pool();
