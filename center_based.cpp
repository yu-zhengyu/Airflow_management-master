...
center_based::center_based(void)
{
	maximum_capacity = 20;
	...
	for(int i=0;i<getIndividualnumber();i++)
		ratio.push_back(0.0);
}
...
void center_based::greedy_schedule(int i){
	vector<int>temp;
...
double center_based::calculate_time_difference(eachAirplane prior, eachAirplane next,double safe_time){
...
void center_based::setSortedconf(int n,double time){
	sortedConf.at(n).setSortedentrancetime(time);
}


bool center_based::kickOverflow(){
...
bool center_based::kickUnsafe(int i){
	vector<int>temp;
	vector<vector<int>>temp_generation = getGeneration_pool();
	temp = temp_generation[i];
	for(int i=0;i<temp.size();i++){
		if((60.0*timeslicesize)/temp[i]<safetime){
			return true;
			break;
		}
		else 
...
double center_based::fitness_calculation(){
	double delay = 0.0;
	for(int i=0;i<sortedConf.size();i++){
...
void center_based::displaySortednumber(){
	for(int i=0;i<sortedNumber.size();i++)
		cout<<sortedNumber[i]<<" ";
}

vector<double> center_based::occupation_ratio(){
	vector<vector<int>>temp_generation = getGeneration_pool();
